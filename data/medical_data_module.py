...
from utils.core_functions import get_preprocessor
...
class StandardMedicalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_indices, val_indices = train_test_split(
            self.df_metadata['isic_id'],
            test_size=1 - self.split_ratio,
            stratify=self.df_metadata['target'],
            random_state=42
        )



        self.preprocessor, features_processed = get_preprocessor(self.df_metadata[(self.df_metadata.index.isin(train_indices))])
        self.processed_features_shape = features_processed.shape
        self.val_indices = self.df_metadata[self.df_metadata['isic_id'].isin(val_indices)].index.tolist()

        # Identify minority and majority class samples
        minority_class = self.df_metadata['target'].value_counts().idxmin()
        self.minority_indices = self.df_metadata[(self.df_metadata.index.isin(train_indices)) & (
                self.df_metadata['target'] == minority_class)].index.tolist()


        minority_count = len(self.minority_indices)
        majority_count = len(train_indices) - len(self.minority_indices)

        self.minority_indices = self.minority_indices * self.training_minority_oversampling_ceoff

        self.majority_indices = self.df_metadata[(self.df_metadata.index.isin(train_indices)) & (
                self.df_metadata['target'] != minority_class)].index.tolist()
        ...
        self.cls_num_list = torch.tensor([majority_count, minority_count],
                                         device="cuda")
