...
class WkNNFeatureSelector(TransformerMixin):
    ...
    def sqeuclidean_dist(self, A, B=None):
        # Efficient computation of squared Euclidean distance using matrix multiplication
        # No need for variable scopes in TensorFlow 2.x
        if B is not None:
            norm_a = tf.reduce_sum(tf.multiply(tf.square(A), self.weights_), axis=1)
            norm_b = tf.reduce_sum(tf.multiply(tf.square(B), self.weights_), axis=1)
            norm_a = tf.reshape(norm_a, [-1, 1])
            norm_b = tf.reshape(norm_b, [1, -1])
            scalar_product = tf.matmul(tf.multiply(A, self.weights_), B, transpose_b=True)
        else:
            norm_a = tf.reduce_sum(tf.multiply(tf.square(A), self.weights_), axis=1)
            norm_b = norm_a
            norm_a = tf.reshape(norm_a, [-1, 1])
            norm_b = tf.reshape(norm_b, [1, -1])
            scalar_product = tf.matmul(tf.multiply(A, self.weights_), A, transpose_b=True)

        D = norm_a - 2 * scalar_product + norm_b

        # Special modification of distance matrix - replace small values with epsilon to avoid computational issues
    ...
    def optimizer_loop(self, optimizer, X, y, n_iter):
        ...
        var_err = tf.Variable(initial_value=0.0, name="var_last_err", dtype=y.dtype)
        var_reg0 = tf.Variable(initial_value=0.0, name="var_last_L0reg", dtype=y.dtype)
        var_reg1 = tf.Variable(initial_value=0.0, name="var_last_L1reg", dtype=y.dtype)
        var_reg2 = tf.Variable(initial_value=0.0, name="var_last_L2reg", dtype=y.dtype)
    ...
    def create_optimizer(self):
        if self.optimizer_ == 'SGD':
            return tf.keras.optimizers.SGD(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'Adam':
            return tf.keras.optimizers.Adam(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'Nadam':
            return tf.keras.optimizers.Nadam(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'Adagrad':
            return tf.keras.optimizers.Adagrad(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'Adadelta':
            return tf.keras.optimizers.Adadelta(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'RMSProp':
            return tf.keras.optimizers.RMSprop(learning_rate=self.learning_rate_)
        elif self.optimizer_ == 'Momentum':
            return tf.keras.optimizers.SGD(learning_rate=self.learning_rate_, momentum=0.9, nesterov=True)
        else:
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(steps):
                try:
                    ...
                    if self.verbose:
                        print('Epoch:', (e + 1) * self.n_iters_in_loop_, 'error:', error, 'L0 reg:', reg0, 'L1 reg:',
                              reg1, 'L2 reg:', reg2)
